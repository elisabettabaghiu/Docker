...
INPUT_CSV  = "metrics_gemma3.csv" # cambia nome LLM
INPUT_JSON = "lookup_table_gemma3.json" # cambia nome LLM
...
labels   = [f"{r['load_level'][0].upper()}-{r['capacity_level'][0].upper()}-{r['latency_risk'][0].upper()}"
            for r in righe]
...
fig.suptitle("Lookup Table — gemma3:latest\n" # cambia nome LLM
             "27 combinazioni (load × capacity × latency_risk) · Docker Model Runner",
...
plt.savefig("grafici_lookup_gemma3.png", dpi=150, bbox_inches="tight") # cambia nome LLM
print("Grafico salvato: grafici_lookup_gemma3.png") # cambia nome LLM

# --- Stampa riepilogo testo ---
print("\nStatistiche gemma3 — 27 combinazioni:")
